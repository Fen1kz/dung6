...
class Cell extends Phaser.Sprite {
  ...
  show() {
    return new Promise((resolve) => {
      ...
      this.game.add.tween(this)
        .to({alpha: 1}, 500, Phaser.Easing.Quadratic.InOut, true)
  ...
  hide() {
    let hideScale = this.game.add.tween(this.scale).to({x: .5, y: .5}, 900, Phaser.Easing.Elastic.InOut, true);
    let hideAlpha = this.game.add.tween(this).to({alpha: 0}, 500, Phaser.Easing.Quadratic.InOut);
    return new Promise((resolve) => {
      hideScale
        .chain(hideAlpha);
